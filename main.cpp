...
int main()
{
	...
	float P_MUT,P_CRU,T_CRU,ale;
	double v_best[1000],v_off[1000],v_on[1000],p_prom[1000],convergencia=0;
...
///////mientras t<maxgen
		while(t<N_GEN)
		{
			...
			p_prom[t]/=N_GEN; //para calculo de curva online
...
///////guardar bbest=gbest€gt-1
			...
			v_best[t-1]+=best.fit; //para curva best-so-far
...
///////normalizacion lineal
			...
			if(convergencia<pob[T_POB-1].fit) //verifica convergencia, deberia estar fuera
			{
				convergencia=pob[T_POB-1].fit;
		 	}
	...
	for(int i=0;i<N_GEN;i++) //calculo best-so-far
		v_best[i]/=N_EXP;
	
	double sum=0;
	for(int i=0;i<N_GEN;i++) //calculo offline
	{
		sum+=v_best[i];
		v_off[i]=(float)sum/(i+1);
	}

	sum=0;
	for(int i=0;i<N_GEN;i++) //calculo online
	{
		sum+=p_prom[i];
		v_on[i]=(float)sum/(i+1);
	}
	
	printf("%-5s%-10s%-10s%-10s\n", "t", "Best", "Online", "Offline"); 
	for(int i=0;i<N_GEN;i++)
		printf("%-5i%-10.7f%-10.7f%-10.7f\n", i, v_best[i], v_on[i], v_off[i]); 
	
	cout<<"converge a: "<<convergencia;
	//getch();
